...
class IRCConnection
  ...
  def IRCConnection.create_tcp_socket(server, port)
    ...
    if @@options[:use_ssl]
      ...
      add_IO_socket(@@socket) {|sock| 
        begin
          events = [];
          sock.readpartial(Buffering::BLOCK_SIZE).split($/).each() {|line|
             events.push(IRCEvent.new(line.chomp))
         }
          events
  ...
  def IRCConnection.do_one_loop
    read_sockets = IO.select(@@readsockets, nil, nil, 0.1);
    if !read_sockets.nil?
      read_sockets[0].each {|sock|
        if sock.eof? && sock == @@socket
          ...
          rescue IRCConnection::NothingToRead
            puts ("Cannont yeild null event")
          end
        end
      }
    end
...
class IRCConnection::NothingToRead < Exception
end
